...
```bash
git clone https://github.com/chrisjohnleah/sage-business-cloud-accounting-api.git
cd sage-business-cloud-accounting-api
composer install
...
- **Match the conventions.** DTOs are `final readonly`, hydrate via `fromArray()` using the typed `MapsAttributes` helpers (never raw array access — PHPStan max forbids it). Requests extend `SageRequest`.
...
2. Add the request in `src/Requests/` (extend `SageRequest`, return `Paginated` for collections).
3. Expose it via a resource in `src/Resources/` (and a `Sage` accessor).
