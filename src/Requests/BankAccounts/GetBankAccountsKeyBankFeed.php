<?php

declare(strict_types=1);

namespace ChrisJohnLeah\SageAccounting\Requests\BankAccounts;

use ChrisJohnLeah\SageAccounting\Data\Reference;
use Saloon\Enums\Method;
use Saloon\Http\Request;
use Saloon\Http\Response;

/**
 * Bank feed endpoint for Banking Cloud Bank accounts
 *
 * GET /bank_accounts/{key}/bank_feed
 */
class GetBankAccountsKeyBankFeed extends Request
{
    protected Method $method = Method::GET;

    /**
     * @param  array<string, mixed>  $filters
     */
    public function __construct(
        private readonly string $key,
        private readonly array $filters = [],
    ) {
    }

    public function resolveEndpoint(): string
    {
        return str_replace('{key}', rawurlencode($this->key), '/bank_accounts/{key}/bank_feed');
    }

    /**
     * @return array<string, mixed>
     */
    protected function defaultQuery(): array
    {
        return array_filter($this->filters, static fn (mixed $v): bool => $v !== null);
    }

    public function createDtoFromResponse(Response $response): ?Reference
    {
        $data = $response->json();

        if (! is_array($data)) {
            return null;
        }

        /** @var array<string, mixed> $data */
        return Reference::fromArray($data);
    }
}
